...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
    ...
    <aside class="lg:col-span-1 space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
            <div class="relative inline-block">
                <img class="h-24 w-24 rounded-full mx-auto object-cover ring-4 ring-pink-50" 
                     src="https://ui-avatars.com/api/?name={{ urlencode($user->full_name) }}&color=7F9CF5&background=EBF4FF">
                @if($user->is_verified)
            ...
            <h3 class="mt-4 font-bold text-gray-900 leading-tight">{{ $user->full_name }}</h3>
        ...
        <nav class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <a href="#" class="block px-6 py-4 text-sm font-medium text-gray-700 hover:bg-pink-50 hover:text-pink-600 border-b border-gray-50 transition">My Preferences</a>
            <a href="#" class="block px-6 py-4 text-sm font-medium text-gray-700 hover:bg-pink-50 hover:text-pink-600 border-b border-gray-50 transition">Received Interests</a>
            <a href="#" class="block px-6 py-4 text-sm font-medium text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition">Shortlisted Profiles</a>
        </nav>
    </aside>
    ...
    <section class="lg:col-span-3 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @forelse($suggestedMatches as $match)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden group hover:border-pink-200 transition">
                    <div class="p-6 flex items-center space-x-4">
                        <div class="h-16 w-16 rounded-xl bg-pink-50 flex-shrink-0 overflow-hidden">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($match->full_name) }}&background=fdf2f8&color=db2777">
                        </div>
                        <div class="flex-1">
                            <h4 class="font-bold text-gray-900 group-hover:text-pink-600 transition">{{ $match->full_name }}</h4>
                            <p class="text-xs text-gray-500 font-medium italic">{{ $match->district ?? 'West Bengal' }}</p>
                    ...
                    <div class="px-6 py-4 bg-gray-50/50 border-t border-gray-50 flex justify-between items-center">
                        ...
                        <button class="bg-pink-600 text-white px-5 py-2 rounded-xl text-xs font-bold hover:bg-pink-700 transition shadow-lg shadow-pink-100">Send Interest</button>
                    </div>
                </div>
            @empty
